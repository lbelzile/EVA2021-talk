...
g1 <- ggplot(data = prof_df %>% filter(country %in% c("IDL","combined")) %>% mutate(country = factor(country)),
             mapping = aes(x = endpt,
                           y = pll)) +
  ...
  geom_text(
    data    = labels_df %>% filter(country %in% c("IDL","combined")) %>% mutate(country = factor(country)),
    hjust   = 1,
    vjust   = -0.1,
    size = 3,
